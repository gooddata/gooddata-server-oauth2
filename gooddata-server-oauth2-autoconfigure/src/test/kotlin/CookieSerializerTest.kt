...
internal class CookieSerializerTest {
    ...
    fun `output is base64 encoded`() {
        ...
        StepVerifier.create(encoded)
            .expectNextMatches { Base64.getDecoder().decode(it.toByteArray()).isNotEmpty() }
            .verifyComplete()
    }
    ...
    fun `invalid base64 value throws error`() {
        ...
        StepVerifier.create(cookieSerializer.decodeCookie(exchange, invalidValue))
    ...
    fun `is possible to decrypt encrypted value`() {
        ...
        StepVerifier.create(cookieSerializer.encodeCookie(exchange, input))
            .assertNext {
                cookieSerializer.decodeCookie(exchange, it).map { transformed ->
    ...
    fun `getAead scenarios with large TTL`() {
        ...
        StepVerifier.create(cookieSerializer.encodeCookie(exchange, ""))
            .assertNext { encodedCookie -> expectThat(encodedCookie).isNotEmpty() }
            .verifyComplete()
    ...
    private fun verifyGetCookieSecurityProperties(client: AuthenticationStoreClient, exactly: Int = 1) {
        verify(exactly = exactly) { client.getCookieSecurityProperties(ORGANIZATION_ID) }
    }
